...
//    int i;
    ...
    switch( ptMsg->cStateTurn) {
        case 'O' : gtSysState.eTurnState = TURNING_NONE; 
        break;
        case 'L' : gtSysState.eTurnState = TURNING_L;
        break;
        case 'R' : gtSysState.eTurnState = TURNING_R;
        break;
    }
...
void LL_MSG_Handle__PublicMessage_saveWhenInInterrupt(unsigned char *pMessage, unsigned long length) { 
    if(sizeof(sgucPublicMessageBuffer) < length) return; // exception: no enough buffer
