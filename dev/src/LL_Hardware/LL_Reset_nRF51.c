...
static void LL_Reset_Reasons_BuzzerBeep(unsigned long ms)
{
    if(0 == LL_TYPE_OF_BUZZER) { 
                ...
                for(int i = 0; i < (ms*1000/666); i++) {
                    LL_LED_ON( E_LL_LED_BUZZER); nrf_delay_us(333);
                    LL_LED_OFF(E_LL_LED_BUZZER); nrf_delay_us(333);
                }
    } else {
                // do ?KHz by buzzer hardware:
                LL_LED_ON( E_LL_LED_BUZZER); nrf_delay_ms(ms);
    }
    // off the buzzer:
    LL_LED_OFF(E_LL_LED_BUZZER);
    nrf_delay_ms(1000);
}
